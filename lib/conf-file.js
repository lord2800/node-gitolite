...
function parseFile(dir, file, container) {
	if(file[0] !== '/') {
...
module.exports = {
	parse: function (dir) {
		...
		result.file = path.join(dir, 'gitolite.conf');
		return parseFile(dir, 'gitolite.conf', result);
	},
	save: function (dir, root) {
		...
		Object.keys(files).forEach(function (file) {
			...
			promises.push(fs.writeFileAsync(file, files[file]));
		});
